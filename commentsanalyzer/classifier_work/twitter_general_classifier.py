...
def main():
    df = pd.read_csv("F://Twitter Data//twitter_general.csv")
    print("CSV loaded")
    print(df.shape)
    df.dropna(inplace=True)
    print(df.isna().sum())
    tk = TweetTokenizer(reduce_len=True)
    df.text = df.apply(lambda row: tk.tokenize(row['text']), axis=1)
    ...
    df.text = [''.join(str(item)) for item in df.text.values]
    ...
    print(report_final)
    dump(classifier_linear, '../twitter_classifier_FINAL_general_v0.joblib')
    dump(vectorizer, '../twitter_vectorizer_general_v0.joblib')
